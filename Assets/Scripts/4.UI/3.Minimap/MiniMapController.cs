...
namespace WutheringWaves
{
    ...
    public class MiniMapController : MonoBehaviour
    {
        ...
        [Tooltip("Bind the world transform that represents the main camera facing direction, not the UI view arrow.")]
        [SerializeField] private Transform viewTarget;
        [Tooltip("Bind the world transform that represents the player's facing direction, not the UI facing arrow.")]
        ...
        private CharacterCore boundCore; // 当前绑定的角色核心
        ...
        public void Initialize(CharacterCore core = null)
        {
            // 初始化时统一补齐相机、角色依赖和默认标记资源。
            CacheCamera();
            BindCharacterCore(core);
        ...
        private void BindCharacterCore(CharacterCore core)
        {
            // 角色核心为空时不覆盖现有引用，避免把已绑定目标清空。
            if (core == null)
            {
                return;
            }
            ...
            {
                followTarget = boundCore.transform;
            }

            if (facingTarget == null)
            {
                facingTarget = boundCore.transform;
            }

            if (viewTarget == null && boundCore.cameraLogic != null && boundCore.cameraLogic.CameraTarget != null)
            {
                viewTarget = boundCore.cameraLogic.CameraTarget.transform;
            }
        }
